                    ...
                    <tr>
                        <th v-for="(column,name, index) in columns" :key="index" class="text-gray-900 font-bold" @click="sortRecords(column)" >
                            <div class="flex items-center justify-between hover:cursor-pointer cursor-pointer email w-full">
                                <span>{{ name }}</span>
                                <fa icon="fa-arrow-down-a-z" v-if= "sortDirection === 'asc' && name != 'Grant'" class= "cursor-pointer"> </fa>
                                <fa icon="fa-arrow-up-z-a" v-if="sortDirection === 'desc' && name != 'Grant'" class="cursor-pointer"></fa>
                            </div>
                        </th>
                    </tr>
                    </thead>
            ...
            <div class="md:w-1/6 mt-6">
                <div class="bg-white rounded shadow p-2">
                    <ul class="text-sm">
                        ...
                        <li class="mb-6"><fa icon="fa-xmark" class="mr-2"/> Denied: {{ permissions.length - form.permissionsArr.length}}</li>
                        <li class="text-right border-t mt-2 "><button type="submit" class="bg-green-500 px-2 py-1 mt-2 rounded text-white">Create Now</button></li>
                    </ul>
                </div>
            </div>
        </div>
